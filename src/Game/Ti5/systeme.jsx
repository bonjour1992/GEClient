...
function Display({ content, style, context, explication }) {
    ...
    return (<div style={{
        ...
        {content.anomaly && <img src={pub + (anomalyPict.get(content.anomaly) || "/404.jpeg")} alt={content.anomaly} style={{ ...systemSize }} />}
        {content.anomaly && content.anomaly.indexOf("hpl") === -1 && <img src={pub + "/ti/system/anomalie.png"} alt="contient anomalie" style={{ ...systemSize, position: "absolute", top: 0, left: 0 }} />}
...
function Form({ content, onChange, onSubmit, style }) {
    ...
    return (
        <FormBase content={content} onChange={onChange} onSubmit={onSubmit} style={style}>
            <BooleanInput onChange={onChange} name="special" value={content} label="Special(exclure de la génération)?" />
